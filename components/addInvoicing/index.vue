		...
		<view class="add_invoicing" :class="invoice.invoice==true?'on':''">
			...
			<form @submit="formSubmit" report-submit="true" >
				...
				<view class="btn-wrap">
					...
					<button class="back" @tap="close">不开发票</button>
				</view>
			</form>
...
<script>
	...
	export default {
		...
		watch: {
			// receipt_title_type(value) {
			// 	this.receipt_title = '';
			// },
			// invoice_id(value) {
			// 	let result = this.invoiceList.find(item => {
			// 		return item.user_receipt_id == value;
			// 	});
			// 	debugger
			// 	this.receipt_title = result.receipt_title;
			// }
		},
		onLoad(option) {
		...
		methods: {
			...
			getInvoiceDefault() {
				...
				invoice(params).then(res => {
					let data = res.data[0];
					this.typeName = data.receipt_type == '1' ? '增值税电子普通发票' : '增值税电子专用发票'
			...
			getInvoiceDetail(id){
				invoiceDetail(id).then(res => {
					...
					this.typeName = this.receipt_type == '1' ? '增值税电子普通发票' : '增值税电子专用发票'
			...
			formSubmit(e) {
				...
				if(value.receipt_title_type == 2){
					...
					if(value.receipt_type == '增值税电子专用发票'){
						...
						if(!/^([1-9]{1})(\d{14}|\d{18})$/.test(value.bank_code)){
