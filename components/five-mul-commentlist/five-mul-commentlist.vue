<template>
	<view class="view-content-comment">
		<!-- <text class="title">{{ title }}</text> -->
		<view class="view-conmment-note" v-if="commentList.length == 0">
			<image
				src="data:image/png;base64,"
				class="icon-none" mode="aspectFit"></image>
			<text class="text-conmment-note">暂无评论</text>
		</view>
		<view v-if="commentList.length > 0" class="view-comment-list" v-for="(item, index) in commentList" :key="index">
			<image :src="item.image_author" class="view-comment-list-item-photo" mode="aspectFill"></image>
			<view class="view-comment-list-item">
				<view class="comment-title">
					<text class="comment-name">{{ item.author }}</text>
					<view style="flex: 1;"></view>
					<image class="praise-icon" @click="clickPraise(item.article_id, 2)"
						:src="item.status == 1?'https://yfl-public.oss-cn-shenzhen.aliyuncs.com/img/zan_s.png':'https://yfl-public.oss-cn-shenzhen.aliyuncs.com/img/zan.png'"
						mode="aspectFill"></image>
					<text class="praise-num" style="margin-left: 10rpx;">{{ item.visit }}</text>
				</view>
				<text class="comment-content" @click="clickRecomment(item)">{{ item.note }}</text>
				<view class="comment-sub">
					<text class="comment-sub-time">{{ item.create_time }}</text>
					<!-- <view class="comment-sub-recomment" style="color: #999999;" v-if="item.CANDELETE == 1"
						@click="clickDelete(item)">删除</view> -->
					<view class="comment-sub-recomment" @click="clickRecomment(item)">回复</view>
				</view>
				<view class="comment-child" v-show="item.list.length > 0">
					<view class="comment-child-item" v-for="(itemChild, indexChild) in item.list"
						:key="indexChild">
						<text class="comment-child-item-name"
							@click="clickRecommentChild(item)">{{ itemChild.author + '：' }}</text>
						<!-- <text class="comment-child-item-content">{{ (itemChild.SECONDNICKNAME ? '回复 ' + itemChild.SECONDNICKNAME + '：' : '') + itemChild.COMMENT }}</text> -->
						<!-- <text class="comment-child-item-content" v-if="itemChild.SECONDNICKNAME">回复</text>
						<text
							class="comment-child-item-content">{{ itemChild.SECONDNICKNAME ? itemChild.SECONDNICKNAME : '' }}</text>
						<text class="comment-child-item-content" v-if="itemChild.SECONDNICKNAME">：</text> -->
						<text class="comment-child-item-content" @click="clickRecommentChild(item)">{{ itemChild.author }}</text>
						<!-- <text v-if="itemChild.CANDELETE == 1" class="comment-child-item-delete"
							@click="clickDeleteChild(itemChild)">删除</text> -->
					</view>
				</view>
			</view>
		</view>
	</view>
</template>

<script>
	export default {
		name: 'MulCommentList',
		//属性
		props: {
			commentList: {
				type: Array
			},
			title: {
				type: String,
				default: '网友评论'
			}
		},
		data() {
			return {};
		},
		onLoad: function(e) {},
		onShow: function() {
			console.log('最新评论', this.title, this.commentList);
		},
		methods: {
			/**
			 * 点赞
			 * @param {Object} item
			 */
			clickPraise(id ,type) {
				this.$emit('clickPraise', id,type);
			},
			/**
			 * 回复 评论
			 * @param {Object} item
			 */
			clickRecomment(item) {
				this.$emit('clickRecomment', item.article_id);
			},
			/**
			 * 删除评论
			 * @param {Object} item
			 */
			clickDelete(item) {
				this.$emit('clickDelete', item);
			},
			/**
			 * 删除评论
			 * @param {Object} item
			 */
			clickDeleteChild(item) {
				this.$emit('clickDeleteChild', item);
			},
			/**
			 * 回复评论的评论
			 * @param {Object} item
			 */
			clickRecommentChild(item) {
				this.$emit('clickRecommentChild', item.article_id);
			}
		}
	};
</script>

<style lang="scss">
	@import '@/common/css/iconfont.css';

	.view-content-comment {
		background-color: white;
		margin-top: 20rpx;
		display: flex;
		flex-direction: column;
		align-items: center;
		padding: 30rpx;

		.title {
			width: 100%;
			color: #333;
			font-weight: bold;
			font-size: 34rpx;
		}
	}

	.praise-icon {
		height: 40rpx;
		width: 40rpx;
		font-size: 36rpx;
		color: #999;
	}

	.praise-num {
		font-size: 26rpx;
		color: #999;
	}

	.view-conmment-note {
		display: flex;
		flex-direction: column;
		justify-content: center;
		align-items: center;
		flex: 1;
		padding-bottom: 20rpx;
	}

	.icon-none {
		width: 200rpx;
		height: 200rpx;
		color: #bbbbbb;
	}

	.text-conmment-note {
		color: #808080;
		font-size: 32rpx;
		margin-top: 30rpx;
	}

	.border-line {
		border-bottom: 1px solid #f6f6f6;
	}

	.view-comment-list {
		width: 100%;
		display: flex;
		flex-direction: row;
		background-color: white;
		padding: 20rpx 0;
		border-bottom: 1px solid #f6f6f6;
	}

	.view-comment-list-item-photo {
		height: 80rpx;
		width: 80rpx;
		border-radius: 40rpx;
		border: 1px solid #f6f6f6;
	}

	.view-comment-list-item {
		margin-left: 16rpx;
		display: flex;
		flex: 1;
		flex-direction: column;
	}

	.comment-name {
		color: #666;
		font-size: 28rpx;
		display: flex;
		align-items: center;
	}

	.comment-title {
		align-items: center;
		display: flex;
		// height: 60rpx;
		flex-direction: row;
	}

	.comment-content {
		padding: 10rpx 0;
		color: #000000;
		font-size: 30rpx;
		font-weight: 500;
	}

	.comment-sub {
		display: flex;
		align-items: center;
		flex-direction: row;
		margin-bottom: 10rpx;

		.comment-sub-time {
			color: #999;
			font-size: 24rpx;
		}

		.comment-sub-recomment {
			color: #666;
			font-size: 24rpx;
			padding: 0 0 0 20rpx;
		}
	}

	.comment-child {
		background-color: #f8f8f8;
		border-radius: 10rpx;
		padding: 0 20rpx;
	}

	.comment-child-item {
		flex-wrap: wrap;
		margin: 20rpx 0;
		display: flex;
		align-items: center;
		flex-direction: row;

		.comment-child-item-name {
			white-space: nowrap;
			font-size: 26rpx;
			color: #5074cf;
		}

		.comment-child-item-content {
			font-size: 26rpx;
			// text-align: center;
		}

		.comment-child-item-delete {
			font-size: 24rpx;
			color: #999999;
			margin-left: 30rpx;
			text-align: center;
		}
	}
</style>
