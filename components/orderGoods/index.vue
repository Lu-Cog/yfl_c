			...
			<view v-for="(item,index) in cartInfo" :key="index">
				...
				<view class='item acea-row row-between-wrapper'>
					...
					<view class='text'>
						...
						<view class='infor line1' ><text >单位：{{item.cart_info.product.unit_name}}</text><text v-if="item.cart_info.productAttr.sku">，规格：{{item.cart_info.productAttr.sku}}</text></view>
						<view class='money font-color' v-if="item.cart_info.product_type ==3">￥{{$util.moneyAct(item.cart_info.productAssistAttr.assist_price)}}</view>
						<view class='money font-color' v-else>￥{{$util.moneyAct(item.cart_info.productAttr.price)}}</view>
