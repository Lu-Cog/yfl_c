		...
		<view class="product-window" :class="(attr.cartAttr === true ? 'on' : '') + ' ' + (iSbnt || destri?'join':'') + ' ' + (isPresell ?'presell-window':'product-window')">
			<view class="textpic acea-row row-between-wrapper">
				...
				<view class="text">
					...
					<view v-else class="money font-color">
						￥<text class="num">{{ $util.moneyAct(attr.productSelect.price) }}</text>
						<text class="stock" v-if='isShow'>库存: {{ attr.productSelect.stock }}</text>
						<text class='stock' v-if="limitNum">限量: {{attr.productSelect.quota_show}}</text>
					</view>
				</view>
			...
			<view class="productWinList">
				<view class="item" v-for="(item, indexw) in attr.productAttr" :key="indexw">
					...
					<view class="listn acea-row row-middle">
						<button  class="itemn" :class="item.index === itemn.attr ? 'on' : ''" v-for="(itemn, indexn) in item.attr_value"
			...
			<view class="cart acea-row">
				...
				<view class="carnum acea-row row-left">
	              ...
	              attr.productSelect.cart_num >= attr.productSelect.stock || destri
	                ? 'on'
	                : ''
	            "
					 @click="CartNumAdd">
					...
					<view v-else class='item plus' :class='(attr.productSelect.cart_num >= attr.productSelect.quota_show) || (attr.productSelect.cart_num >= attr.productSelect.product_stock)? "on":""'
					 @click='CartNumAdd'>+</view>
				</view>
			</view>
			<view class="joinBnt2 bg-color" v-if="isAddCart" @tap="joinCart()">加购物车</view>
			<view class="joinBnt bg-color" v-if="destri && attr.productSelect.stock>0" @click="goCat">立即购买</view>
			<view class="joinBnt on" v-else-if="destri && attr.productSelect.stock<=0">已售罄</view>
			<view class="joinBnt bg-color" v-if="iSbnt && attr.productSelect.product_stock>0 &&attr.productSelect.quota>0"
...
<script>
	export default {
		...
		methods: {
			joinCart:function(){
				console.log("buyProId="+this.buyProId);
			    this.$emit("pjoinCart",this.buyProId)
			},
			//  查看大图
			loookImg(){
				let self = this
