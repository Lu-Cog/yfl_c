<template>
	<view class="product-con">
		...
		<view>
			<scroll-view :scroll-top="scrollTop" scroll-y='true' scroll-with-animation="true" :style='"height:"+height+"px;"'
			 @scroll="scroll">
				<view id="past0">
					...
					<view class='wrapper'>
						...
						<view class='label acea-row'>
							<view>原价:￥{{storeInfo.price || ''}}</view>
							<view style="margin-left: 30rpx;">已预定:{{presellInfo.tattend_one && presellInfo.tattend_one.pay}}{{ storeInfo.unit_name }}</view>
						</view>
						<view class='introduce'>{{presellInfo.store_name}}</view>

						<view class="presell_count">
							<view>
								<view>预售活动时间：</view>
								<view class="presell_time"> <view class='iconfont icon-shijian1'></view> {{ presellInfo.start_time }} <span class='area_line'>~</span><view class='iconfont icon-shijian1'></view> {{ presellInfo.end_time }}</view>
							</view>
							<view v-if="presellInfo.presell_type === 2">
								<view>支付尾款时间：</view>
								<view class="presell_time"> <view class='iconfont icon-shijian1'></view> {{ presellInfo.final_start_time }} <span class='area_line'>~</span><view class='iconfont icon-shijian1'></view> {{ presellInfo.final_end_time }}</view>
							</view>
							...
							<view v-if="presellInfo.delivery_type == 2">预售结束后{{ presellInfo.delivery_day }}天内发货</view>
				...
				<!-- 商铺信息 -->
				<view v-if="hide_mer_status != 1 && storeInfo.merchant" class="store-wrapper">
					<view class="store-hd">
		...
		<view class='footer_count'>
			<view v-if="presellInfo.presell_type === 2 && presellInfo.final_end_time" class="presell_desc">
			...
			<view class="acea-row row-between-wrapper footer" :class="presellInfo.presell_type === 2 ? 'noborder' : ''">
				<navigator hover-class="none" class="item" :url="`/pages/chat/customer_list/chat?mer_id=${storeInfo.mer_id}&uid=${uid}`">
				...
				<view class='bnt acea-row'>
					<form @submit="goBuy" report-submit='true' v-if="attr.productSelect && presellInfo.presell_status == 1">
						...
						<button v-else class='buy bnts' form-type="submit">{{ presellInfo.presell_type === 1 ? "立即支付" : "立即支付定金："+minNum+'起'  }}</button>
					</form>
		...
		<!-- 组件 -->
		<productWindow :attr="attr" :isShow='1' :iSplus='1' :isPresell='1' :presell_type="presellInfo.presell_type" :image="storeInfo.image" @myevent="onMyEvent" @ChangeAttr="ChangeAttr" @ChangeCartNum="ChangeCartNum"
	...
	export default {
		...
		methods: {
			...
			ChangeCartNum: function(changeValue) {
				...
				let stock = this.presellInfo.self_count?this.presellInfo.self_count:999999999;
			...
			getGoodsDetails: function() {
				...
				let that = this;
				getPresellProductDetail(that.id).then(res => {
					console.log(res)
					uni.hideLoading();
					...
					that.$set(that, 'replyCount', res.data.product.replayData.count);
					that.$set(that, 'replyChance', res.data.product.replayData.rate);
					...
					this.minNum = Math.min.apply(Math, m.map(function(o) {return o.down_price}))
			...
			setCollect: function() {
				if (this.isLogin === false) {
					...
					if (this.storeInfo.isRelation) {
						collectDel({
							type: 1,
							type_id: this.presellInfo.product_id
						}).then(res => {
			...
			downloadFilePromotionCode: function(successFn) {
				...
				type = 'routine'
				// #endif
				getProductCode(that.id,{type: type, product_type: 2}).then(res => {
					uni.downloadFile({
						// #ifdef MP
...
<style scoped lang="scss">
	...
	.icon-zhuyi-copy{display: inline-block; margin-right: 5rpx; position: relative; top: 3rpx;}
</style>
