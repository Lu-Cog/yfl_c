		...
		<view class='flash-sale'>
			<view class='list'>
				<view v-for="(item,index) in broadcastList" :key='index' >
					...
					<navigator hover-class="none" :url="((item.live_status == 103 && item.replay_status) || item.live_status ===
								101 || item.live_status ===
								102) ?
							'plugin-private://wx2b03c6e691cd7370/pages/live-player-plugin?room_id=' + item
							.room_id + '&custom_params=' + param : ''">
...
<script>
	...
	export default {
		computed: mapGetters(['uid','store_id']),
		components: {
			home
		},
		data() {
		...
		onShow() {
			...
			this.param = encodeURIComponent(JSON.stringify(data))
		},
		methods: {
			getBroadcastList() {
				var that = this;
