<template>
	<view>
		<view class='payment-status'>
			...
			<view class='iconfont icon-duihao2 bg-color icon' ></view>
			...
			<!-- 失败时：订单支付失败 -->
			<view class='status' v-if="order_pay_info.paid == 0">下单成功，订单未支付</view>
			<view class='status' v-else-if="order_pay_info.paid == 1">下单成功，支付成功</view>
			<view class='status' v-else>下单成功，订单部分付款</view>
			<view class='wrapper'>
				<view class='item acea-row row-between-wrapper'>
					<view>订单编号</view>
					<view class='itemCom'>{{order_pay_info.son_order_sn}}</view>
				</view>
				<view class='item acea-row row-between-wrapper'>
					<view>下单时间</view>
				...
				<view class='item acea-row row-between-wrapper'>
					<view>订单总金额</view>
					<view class="itemCom font-color-red">￥{{$util.moneyAct(order_pay_info.pay_price)}}</view>
				</view>
				<!-- <view class='item acea-row row-between-wrapper'>
					<view>支付方式</view>
					<view class='itemCom'>余额支付</view> -->
				...
				<!--失败时加上这个  -->
				<!--<view class='item acea-row row-between-wrapper' v-if="order_pay_info.paid != 0 ">
					<view>支付方式</view>
					<view class="itemCom">余额支付</view>
				</view> -->
				<block v-if="order_pay_info.paid == 1 ">
					<view class='item acea-row row-between-wrapper' >
						<view>已支付商品金额</view>
						<view class="itemCom font-color-red">￥{{$util.moneyAct(order_pay_info.total_price)}}</view>
					</view>
				</block>
				<block v-else>
					<view class='item acea-row row-between-wrapper' v-if="order_pay_info.paid_price > 0 && order_pay_info.total_price != order_pay_info.paid_price ">
						<view>已支付商品金额</view>
						<view class="itemCom font-color-red">￥{{$util.moneyAct(order_pay_info.paid_price )}}</view>
					</view>
					<view class='item acea-row row-between-wrapper' v-if="order_pay_info.total_price != order_pay_info.paid_price ">
						<view>待支付商品金额</view>
						<view class="itemCom font-color-red">￥{{$util.moneyAct(order_pay_info.total_price - order_pay_info.paid_price )}}</view>
					</view>
				
				</block>
				
				<view class='item acea-row row-between-wrapper' v-if="order_pay_info.paid_postage>0 ">
					<view>已支付运费</view>
					<view class="itemCom font-color-red">￥{{$util.moneyAct(order_pay_info.total_postage)}}</view>
				</view>
				<view class='item acea-row row-between-wrapper' v-else-if="order_pay_info.total_postage>0">
					<view>待支付运费</view>
					<view class="itemCom font-color-red">￥{{$util.moneyAct(order_pay_info.total_postage)}}</view>
				</view>
			</view>
