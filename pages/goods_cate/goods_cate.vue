		...
		<view class="con-box">
			<view class='aside'>
				<scroll-view scroll-y="true" style="height: 100%; overflow: hidden;" scroll-with-animation='true' :scroll-into-view="intoindex">
				 ...
				 :key="index" @click='tap(index,"b"+index)' :id='"sort"+index'><text class="item_text">{{item.cate_name}}</text></view>
				</scroll-view>
			</view>
			<view class='conter' v-if="productList.length>0">
				<scroll-view scroll-y="true" style="height: 100%; overflow: hidden;" @scroll="scroll" scroll-with-animation='true'>
					<block  v-if="productList[navActive]">
						<view class='listw' :id="'b'+index" v-for="(item,index) in productList[navActive].children" :key="index">
