<template>
	<view class="product-con">
			...
			<scroll-view :scroll-top="scrollTop" scroll-y='true' scroll-with-animation="true"
				...
				<view id="past0">
					...
					<view class='wrapper'>
						<view class='share acea-row row-between row-bottom'>
							<view class='money font-color'>
								￥ <text class='num'>{{$util.moneyAct(storeInfo.price)}}</text>
								<text class="goods_tag" v-if="activity_type_tip">{{activity_type_tip}}</text>
								<!-- <text class='vip-money' v-if="storeInfo.vip_price && storeInfo.vip_price > 0">￥{{storeInfo.vip_price}}</text> -->
								<!-- <image v-if="storeInfo.hd_price && storeInfo.hd_price > 0" src="/static/images/vip.png"></image> -->
							</view>

							<view v-if="room_id==''" class='iconfont icon-fenxiang' @click="listenerActionSheet"></view>
						</view>
						<view class='introduce'><text v-if="storeInfo.merchant.is_trader"
								class="font-bg-red">自营</text>{{storeInfo.store_name ? storeInfo.store_name : ''}}</view>
						<view class='label acea-row row-between-wrapper'
							:style="{'padding-bottom':coupon.list.length?0:10+'px'}">
							<view v-if="activity_type_tip">原价:<label
									class="old_price">￥{{$util.moneyAct(storeInfo.ot_price)}}</label></view>
							<view>
					...
					<view class='attribute acea-row row-between-wrapper' @click="selecAttr">
						<view>{{attrTxt}}：
							<text v-show="attrValue" class='atterTxt'>{{attrValue}}</text>
						</view>
						<view class='iconfont icon-jiantou'></view>
					</view>
				</view>
				...
				<!-- <view class="store-wrapper" v-if="storeInfo.merchant && hide_mer_status !=1">
					<view class="store-hd">
						<view class="store-info">
							...
							<view class="info">
								<view class="name">{{storeInfo.merchant.mer_name ? storeInfo.merchant.mer_name : ''}}<text v-if="storeInfo.merchant.is_trader" class="font-bg-red ml8">自营</text></view>
								<view class="txt">{{storeInfo.merchant.care_count ? storeInfo.merchant.care_count : ''}}人关注</view>
							</view>
				...
				<view class='product-intro' id="past3">
					...
					<view class='conter' v-if="description">
						<jyf-parser :domain='domain' :html="description.content.replace(/<br\/>/ig, '')" ref="article"
		...
		<view class='footer acea-row row-between-wrapper'>
			<button class="item" open-type="contact" @contact='contact' @click="couponTap2">
			...
			<!-- <navigator open-type='switchTab' class="animated item" :class="animated==true?'bounceIn':''" url='/pages/order_addcart/order_addcart'
			 hover-class="none"> -->
			<navigator class="animated item" :class="animated==true?'bounceIn':''"
				@click="goCart" hover-class="none">
			...
			<view class='bnt acea-row'>
				<form @submit="joinCart" report-submit='true' v-if="attr.productSelect">
					<button class='joinCart bnts' form-type="submit"
						:disabled="attr.productSelect.stock == 0">加入购物车</button>
				</form>
...
<script>
	...
	export default {
		...
		computed: mapGetters(['isLogin', 'uid','stype']),
		onLoad(options) {
			if(options.room_id){
			...
			if (options.mer_id) app.globalData.mer_id = options.mer_id;
			// #endif
		...
		methods: {
			...
			getGoodsDetails: function() {
				...
				getProductDetail(that.id,data).then(res => {
					...
					if (storeInfo.activity_type == 1 || storeInfo.activity_type == 2) {
						that.activity_type_tip = "活动价";
					} else if (storeInfo.activity_type == 3) {
						that.activity_type_tip = "老代理";
					} else if (storeInfo.activity_type == 4) {
						that.activity_type_tip = "YBK老代理";
					} else if (storeInfo.activity_type == 5) {
						that.activity_type_tip = "特定价";
					} else {
						// that.activity_type_tip = "市场价";
					}
					// #ifdef MP
					if (storeInfo.video_link.indexOf('https') == -1) {
						storeInfo.video_link = `https:${storeInfo.video_link}`
					}
					// #endif
