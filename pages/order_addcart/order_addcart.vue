	...
	<view>
		<view class='shoppingCart'>
			...
			<view class='nav acea-row row-between-wrapper'>
				<view>购物车数量 <text class='num font-color show_num'> {{cartTotalCount}}</text></view>
					...
					<block v-for="(item,index) in cartList.valid" :key="index">
						<view class='item acea-row row-between-wrapper'>
							<view class="store-title">
								...
								<!-- <navigator :url="hide_mer_status != 1 ? '/pages/store/home/index?id='+item.mer_id : '#'" class="info"> -->
							...
							<navigator v-for="goods in item.list" :key="goods.cart_id" :url='"/pages/goods_details/index?id="+goods.product.product_id'
							 hover-class='none' class='picTxt acea-row'>
								...
								<view class='text'>
									...
									<view class='infor line1' ><text >单位：{{goods.product.unit_name}}</text><text v-if="goods.productAttr.sku">，规格：{{goods.productAttr.sku}}</text></view>
									<view class='money'>￥{{$util.moneyAct(goods.productAttr.price)}}
							
									<text v-if="goods.productAttr.activity_type==1 || goods.productAttr.activity_type==2 " class="goods_tag">活动价</text>
									...
									<!-- <text v-else-if="goods.productAttr.activity_type==0" class="goods_tag">市场价</text> -->
			...
			<view class='footer acea-row row-between-wrapper' v-if="cartList.valid.length > 0">
				...
				<view class='money acea-row row-middle' v-if="footerswitch==true">
					<text class='font-color show_num'>￥{{$util.moneyAct(selectCountPrice)}}</text>
