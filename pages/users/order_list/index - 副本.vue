		...
		<view class='my-order'>
			<view class='header bg-color'>
				<view class='picTxt acea-row row-between-wrapper'>
					<view class='text'>
						...
						<!-- <view>订货订单：{{orderData.orderCount || 0}} 总消费：￥{{orderData.orderPrice || 0}}</view> -->
					</view>
					<view class='pictrue'>
			...
			<view class='nav acea-row row-around'>
				<view class='item' :class='orderStatus==-1 ? "on": ""' @click="statusClick(-1)">
					<view>全部</view>
					<view class='num'>{{orderData.noPay+orderData.noPostage || 0}}</view>
				</view>
					 ...
					 <view class='item' v-for="(item,index) in orderList" :key="index">
					 		<view @click='goOrderDetails(item.group_order_id)'>
					 			<view class='title acea-row row-between-wrapper'>
					 				...
					 				<view v-if="item.paid == 2" class='font-color'>待部分付款</view>
					 				<view v-if="item.paid == 1" class='font-color'>待发货</view>
					 			...
					 			<view v-if="item.activity_type === 2">
					 				<view class='item-info acea-row row-between row-top' v-for="(goods,index) in item.orderProduct" :key="index">
					 					...
					 					<view class='text acea-row row-between'>
					 						...
					 						<view class='money'>
					 							<view>￥{{$util.moneyAct(goods.cart_info.productPresellAttr.presell_price)}}</view>
					 		...
					 		<view class='bottom acea-row row-right row-middle'>
								...
								<view class='bnt bg-color' @click='goOrderDetails(item.group_order_id)'>查看详情</view>
							</view>
					...
					<view class='item' v-for="(item,index) in orderList" :key="index">
						...
						<view class='bottom acea-row row-right row-middle'>
							...
							<block v-if="item.status == 1">
								<view class='bnt cancelBnt' @click='goOrderDetails(item.group_order_id)'>查看物流</view>
							...
							<block v-if="item.status == 3">
								...
								<view class='bnt bg-color' @click='goOrderDetails(item.group_order_id)' v-if="item.activity_type == 2 || item.activity_type == 3">查看详情</view>
