	...
	<view>
		<form @submit="formSubmit" report-submit="true">
			...
			<view v-show="receipt_title_type == '2' && receipt_type == '2'" class="panel">
				<view class="acea-row row-middle">
					<view>开户银行</view>
					<input name="bank_name" :value="bank_name" maxlength="15" placeholder="您的开户银行" />
				</view>
				<view class="acea-row row-middle">
					<view>银行账号</view>
			...
			formSubmit(e) {
				...
				if (formData.receipt_title_type === '2') {
					...
					if(this.receipt_type === 2){
						if(!/^([1-9]{1})(\d{14}|\d{18})$/.test(formData.bank_code)){
							return this.$util.Tips({
								title: '请输入正确的银行账号'
							});
						}
					...
					// if (!/[0-9A-HJ-NPQRTUWXY]{2}\d{6}[0-9A-HJ-NPQRTUWXY]{10}/.test(formData.duty_paragraph)) {
					// 	return this.$util.Tips({
					// 		title: '请正确输入纳税人识别码'
					// 	});
					// }
